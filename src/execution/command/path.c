...
/*   Updated: 2025/02/05 06:55:29 by ipetrov          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *get_pathname(t_node *node)
{
	...
	if (is_pathname(node->ctx->stash[0]))
	{
		...
		if (!pathname)
			error(node, STRUCT_NODE, MALLOC);
		if (!is_exist(pathname))
			error(node, STRUCT_NODE, FILE_NOT_FOUND); // bash: ./test/lds: No such file or directory
	}
	else
	{
		pathval = get_varvalue(node->ctx, "PATH");
		if (!pathval)
			error(node, STRUCT_NODE, FILE_NOT_FOUND);
		pathname = retrieve_pathname(pathval, node);
		if (!pathname)
			error(node, STRUCT_NODE, CMD_NOT_FOUND); //exit(127); inside of error // bash: dfdf: command not found
	}
	if (is_executable(pathname))
		return (pathname);
	free(pathname);
	error(node, STRUCT_NODE, PERMISSION_DENIED); // bash: ./test/ls: Permission denied
	return (NULL);
}
